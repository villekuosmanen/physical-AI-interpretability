...
class MultiModalSAE(nn.Module):
    ...
    def _ensure_correct_dtype(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.use_bfloat16 and x.dtype != torch.float32:
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(x.shape) == TENSOR_DIM_3D:  # (batch_size, num_tokens, token_dim)
    ...
    def compute_loss(
        ...
        with torch.no_grad():
            ...
            feature_sparsity = (torch.abs(features_loss) < SPARSITY_THRESHOLD).float().mean()
